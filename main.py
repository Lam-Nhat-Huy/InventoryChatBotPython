...
def find_equipment_inventory(equipment_name):
    ...
    try:
        with connection.cursor() as cursor:
            # Tìm kiếm tất cả các lô của thiết bị trong cơ sở dữ liệu
            sql = """
            ...
            if results:
                # Tổng hợp tất cả các lô
                total_quantity = sum(item['current_quantity'] for item in results)
                return jsonify({
                    "equipment_name": results[0]['equipment_name'],
                    "equipment_code": results[0]['equipment_code'],
